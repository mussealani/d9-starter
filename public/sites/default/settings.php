 ...
 *
 * This will load the settings file for the environment that has been
 * configured in .env, if it exists, and also the settings.local.php file.
 * The local file will be loaded at the end and could be used to override any
 * variables for the local environment. This file isn't kept in git, so it's
 * safe to do any modifications that you'd like.
 *
 * Keep this code block at the end of this file to take full effect.
 */
foreach ([$_ENV['APP_ENV'], 'local'] as $environment) {
  $settings_file_path = "{$app_root}/sites/default/{$environment}.settings.php";
  if (file_exists($settings_file_path)) {
    include $settings_file_path;
  }

  $services_file_path = "{$app_root}/sites/default/{$environment}.services.yml";
  if (file_exists($services_file_path)) {
    $settings['container_yamls'][] = $services_file_path;
  }
}
